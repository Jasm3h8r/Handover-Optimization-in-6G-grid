...
class ContextualLinUCB:
    """Contextual Multi-Armed Bandit using LinUCB"""
...
class HetNet6GSimulator:
    ...
    def _initialize_hetnet(self):
        ...
        self.cmab = ContextualLinUCB(
            n_arms=self.num_bs, 
            n_features=10,  # Extended feature set for 6G
    ...
    def _get_context(self, user: UserEquipment6G, bs: BaseStation6G,
                    current_loads: Dict[int, int] = None) -> np.ndarray:
        """Enhanced context vector for 6G"""
        d = self._distance_3d(user.x, user.y, user.z, bs.x, bs.y, bs.z)
        azimuth = self._angle_between(bs.x, bs.y, user.x, user.y)
        elevation = self._elevation_angle(bs.x, bs.y, bs.z, user.x, user.y, user.z)
        ...
        context = np.array([
            ...
            (azimuth + 180) / 360,
            (elevation + 90) / 180,
            bs.freq / 150,  # Normalized for Sub-THz
            bs.power / 50,
            bs.bandwidth_mhz / 5000,  # Normalized for max 5 GHz
            bs.num_antennas / 512,
            current_load / max(self.num_ues / self.num_bs * 2, 1),
            1.0 if bs.freq > 90 else 0.5 if bs.freq > 20 else 0.0  # Band indicator
        ])
    ...
    def run_simulation(self, num_steps: int = 300) -> Dict:
        ...
        print(f"\n{'='*80}")
        print(f"6G GROUND HETNET SIMULATION")
        print(f"{'='*80}")
        ...
        print(f"  [10] 6G metrics: Cell-edge rate, outage prob, ping-pong rate")
        print(f"{'='*80}\n")
    ...
    def print_summary(self):
        ...
        print(f"\n{'='*80}")
        print("6G HETNET SIMULATION SUMMARY")
        print(f"{'='*80}")
